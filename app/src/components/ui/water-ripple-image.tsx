...
export default function WaterRippleImage({
  ...
  // parameters state (mirrors original `params`)
  ...
  const resize = () => {
    ...
    const w = Math.floor((parent ? parent.clientWidth : window.innerWidth) * dpr);
    const h = Math.floor((parent ? parent.clientHeight : window.innerHeight) * dpr);
  ...
  // Sync prop src with state when prop changes
  useEffect(() => {
    setParams((prev) => ({ ...prev, src }));
    const gl = glRef.current;
    if (gl) {
      loadImage(src, gl).catch((e) => console.error(e));
    }
  }, [src]);
  ...
  // when params change → push to uniforms
  useEffect(() => {
    const gl = glRef.current;
    if (!gl) return;
    updateUniforms(gl);
  ...
  return (
    <div className={`relative w-full h-full overflow-hidden ${className}`}>
      ...
      <canvas
        ref={canvasRef}
        className="absolute inset-0 size-full block object-cover"
      />
    </div>
  );
}
