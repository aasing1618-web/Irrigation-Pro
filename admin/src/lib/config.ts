/**
 * Configuration du dashboard, résolue une seule fois au démarrage.
 *
 * Même contrôle qu'en Vague 0 pour l'application cliente, et pour la même
 * raison — « HTTPS partout » : ce sont les identifiants du propriétaire, les
 * adresses e-mail de ses clients et les mots de passe temporaires qu'il vient
 * de tirer qui circulent sur ce lien. On échoue bruyamment plutôt que de
 * dégrader en silence.
 *
...
export function resolveApiUrl(rawValue: string | undefined, isDev: boolean): string {
  ...
  throw new ConfigurationError(
    `Connexion non sécurisée refusée : l'adresse du serveur doit utiliser « https:// » ` +
      ...
      `le dashboard n'accepte aucune communication non chiffrée.`,
  );
}
...
/**
 * Renvoie la configuration effective. Lève une `ConfigurationError` si le
 * dashboard est mal configuré — l'erreur est rattrapée en haut de l'arbre React
 * et présentée sous une forme lisible.
 */
